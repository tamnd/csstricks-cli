...
type App struct {
	client *csstricks.Client
	cfg    csstricks.Config
...
func Root() *cobra.Command {
	app := &App{cfg: csstricks.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cst",
		Short: "Browse CSS-Tricks web development articles",
		Long: `cst reads CSS-Tricks through its public RSS feed.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

cst is an independent tool and is not affiliated with CSS-Tricks or DigitalOcean.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = csstricks.NewClient(a.cfg)
	return nil
}
