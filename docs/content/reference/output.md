...
describes what users get. Pick a format with `-o`, or let cst choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cst <command> -o table   # aligned columns for reading
cst <command> -o jsonl   # one JSON object per line, for piping
cst <command> -o json    # a single JSON array
cst <command> -o csv     # spreadsheet friendly
cst <command> -o tsv     # tab-separated
cst <command> -o url     # just the URL column
cst <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cst <command> --fields id,title,url
```
...
```bash
cst <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cst <command>            # a table, because this is a terminal
cst <command> | wc -l    # JSONL, because this is a pipe
```
